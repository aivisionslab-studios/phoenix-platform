...
async function startServer() {
  ...
  app.get("/api/state", async (_req, res) => {
    try {
      const ctrl = new AbortController();
      // PHX-FIX (2026-09-07, achado real do usuário: em hardware sob carga
      // o Engine podia responder em ~5s, mas o proxy abortava em 2s -
      // resultado: dashboard mostrava "RAG DOCS 0", hardware "aguardando
      // descoberta", tudo OFFLINE, mesmo com localhost:8000/api/state
      // saudável e respondendo). 2000 -> 15000.
  ...
  app.post("/api/dual-collab", async (req, res) => {
    ...
    const { topic, max_rounds, cpu_model, gpu_model, use_rag, rag_n_results, rag_min_score } = req.body || {};
    ...
    try {
      ...
      const r = await fetch(`${PHOENIX_ENGINE_URL}/api/dual-collab`, {
        ...
        body: JSON.stringify({
          ...
          rag_n_results: rag_n_results || 5,
          rag_min_score: typeof rag_min_score === "number" ? rag_min_score : 0.15,
        }),
        signal: controller.signal,
        dispatcher: longRunningDispatcher,
      } as RequestInit & { dispatcher: UndiciAgent });
  ...
  app.get("/api/rag", async (_req, res) => {
    ...
    try {
      const ctrl = new AbortController();
      // PHX-FIX (2026-09-07): mesmo motivo do /api/state acima - 2000 -> 15000.
      const t = setTimeout(() => ctrl.abort(), 15000);
      const r = await fetch(`${PHOENIX_ENGINE_URL}/api/state`, { signal: ctrl.signal });
      clearTimeout(t);
      if (r.ok) {
  ...
  app.post("/api/rag", async (req, res) => {
    ...
    try {
      ...
      return res.status(isAbort ? 504 : 502).json({ success: false, error: isAbort ? "Tempo esgotado indexando o conteúdo no RAG (15min)." : `Phoenix Engine indisponível: ${describeFetchError(e)}` });
    }
  });
  ...
  app.post("/api/rag/add-file-async", ragUpload.single("file"), async (req, res) => {
    const file = req.file;
    ...
    try {
      ...
      const r = await fetch(`${PHOENIX_ENGINE_URL}/api/rag/add-file-async`, {
        method: "POST", body: form, dispatcher: longRunningDispatcher,
      } as any);
      const data: any = await r.json().catch(() => ({}));
      if (r.ok) return res.status(202).json({ ...data, success: true });
      return res.status(r.status).json({ success: false, error: data.detail || "Falha ao iniciar indexação RAG." });
    } catch (e: any) {
      return res.status(502).json({ success: false, error: `Phoenix Engine indisponível: ${describeFetchError(e)}` });
    }
  });

  // PHX-PHASE6Q: proxy único e read-only para progresso agregado de
  // workflows. Arena, audiolivro, RAG e provisioning continuam donos do
  // próprio lifecycle; esta rota apenas expõe o registry canônico da 6P.
  app.get("/api/workflows/progress", async (req, res) => {
    try {
      const qs = new URLSearchParams();
      if (typeof req.query.include_idle === "string") qs.set("include_idle", req.query.include_idle);
      if (typeof req.query.limit === "string") qs.set("limit", req.query.limit);
      const suffix = qs.toString() ? `?${qs.toString()}` : "";
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), 2000);
      const r = await fetch(`${PHOENIX_ENGINE_URL}/api/workflows/progress${suffix}`, { signal: ctrl.signal });
      clearTimeout(t);
      const data: any = await r.json().catch(() => ({}));
      if (r.ok) return res.json(data);
      return res.status(r.status).json({ available: false, active_count: 0, total_count: 0, active: [], workflows: [], error: data.detail || "Falha ao consultar progresso dos workflows." });
    } catch (e: any) {
      return res.status(502).json({ available: false, active_count: 0, total_count: 0, active: [], workflows: [], error: `Phoenix Engine indisponível: ${describeFetchError(e)}` });
    }
  });

  app.get("/api/rag/jobs/:jobId", async (req, res) => {
    try {
      const r = await fetch(`${PHOENIX_ENGINE_URL}/api/rag/jobs/${encodeURIComponent(req.params.jobId)}`);
      const data: any = await r.json().catch(() => ({}));
      if (r.ok) return res.json(data);
      return res.status(r.status).json({ ok: false, error: data.detail || "Falha ao consultar job RAG." });
    } catch (e: any) {
      return res.status(502).json({ ok: false, error: `Phoenix Engine indisponível: ${describeFetchError(e)}` });
    }
  });
  ...
  // PHX-NEW (2026-09-07, pedido explícito do usuário: "podemos criar um
  // reset em estado de fábrica... todas as travas são zeradas, mas nunca
  // abertas"). Timeout maior (60s) porque a operação real inclui apagar e
  // recriar a coleção inteira do Chroma - pode demorar mais que uma
  // consulta/upload normal em repositórios grandes.
  app.post("/api/rag/reset", async (req, res) => {
    const confirm = String(req.body?.confirm || "");
    try {
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), 900_000);
      const r = await fetch(`${PHOENIX_ENGINE_URL}/api/rag/reset`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data: any = await r.json().catch(() => ({}));
      if (r.ok) return res.json(data);
      return res.status(r.status).json({ ok: false, error: data.detail || "Falha ao resetar o RAG." });
    } catch (e: any) {
      const isAbort = e?.name === "AbortError";
      return res.status(isAbort ? 504 : 502).json({ ok: false, error: isAbort ? "Tempo esgotado resetando o RAG (15min)." : `Phoenix Engine indisponível: ${describeFetchError(e)}` });
    }
  });
  ...
  // API: Text Execution Mode — padrão/CPU/GPU/híbrido para o LLM
  // compartilhado. O backend persiste a escolha e reinicia o llama.cpp
  // ativo com o mesmo modelo quando necessário.
  app.get("/api/engine/text-execution", async (_req, res) => {
    try {
      const r = await fetch(`${PHOENIX_ENGINE_URL}/api/engine/text-execution`);
      const data = await r.json().catch(() => null);
      if (r.ok && data) return res.json(data);
      return res.status(r.status || 502).json({ error: (data && (data as any).detail) || `Phoenix Engine devolveu status ${r.status}.` });
    } catch (err: any) {
  ...
  app.post("/api/engine/text-execution", async (req, res) => {
    try {
      const controller = new AbortController();
      ...
      try {
        ...
        const data = await r.json().catch(() => null);
        if (r.ok && data) return res.json(data);
        return res.status(r.status || 502).json({ error: (data && (data as any).detail) || `Phoenix Engine devolveu status ${r.status} ao trocar modo de execução.` });
      } finally {
        clearTimeout(timeoutId);
      }
    } catch (err: any) {
      return res.status(502).json({ error: `Falha ao aplicar modo de execução: ${describeFetchError(err)}` });
    }
  });
