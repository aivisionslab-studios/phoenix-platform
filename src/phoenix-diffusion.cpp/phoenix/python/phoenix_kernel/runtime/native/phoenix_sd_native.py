...
def write_png(path: str | os.PathLike[str], width: int, height: int, channels: int, pixels: bytes) -> None:
    """Small dependency-free PNG writer for 8-bit RGB/RGBA/gray output."""
    if channels not in (1, 3, 4):
        raise ValueError(f"Unsupported channel count for PNG: {channels}")
...
class PhoenixSdNative:
    """Direct ctypes binding to phoenix_sd_bridge; no CLI, HTTP server, or subprocess."""
    ...
    def __init__(self, dll_path: str | os.PathLike[str]):
        ...
        if not self._handle:
            raise PhoenixSdError("phx_sd_create returned null")
        self._closed = False

    def _configure_abi(self) -> None:
        d = self._dll
        d.phx_sd_create.restype = ctypes.c_void_p
        d.phx_sd_destroy.argtypes = [ctypes.c_void_p]
        d.phx_sd_load_model.argtypes = [ctypes.c_void_p, ctypes.POINTER(_LoadConfig)]
        d.phx_sd_load_model.restype = ctypes.c_int32
        d.phx_sd_is_loaded.argtypes = [ctypes.c_void_p]
        d.phx_sd_is_loaded.restype = ctypes.c_int32
        d.phx_sd_unload_model.argtypes = [ctypes.c_void_p]
        d.phx_sd_generate.argtypes = [ctypes.c_void_p, ctypes.POINTER(_GenerateConfig), ctypes.POINTER(_Image)]
        d.phx_sd_generate.restype = ctypes.c_int32
        d.phx_sd_free_image.argtypes = [ctypes.POINTER(_Image)]
        d.phx_sd_cancel.argtypes = [ctypes.c_void_p]
        d.phx_sd_last_error.argtypes = [ctypes.c_void_p]
        d.phx_sd_last_error.restype = ctypes.c_char_p
        d.phx_sd_bridge_version.restype = ctypes.c_char_p
        d.phx_sd_system_info.restype = ctypes.c_char_p
        d.phx_sd_physical_cores.restype = ctypes.c_int32
    ...
    def load_model(self, cfg: ModelConfig) -> None:
        native = _LoadConfig(
            _b(cfg.model_path), _b(cfg.diffusion_model_path), _b(cfg.clip_l_path), _b(cfg.clip_g_path),
            _b(cfg.t5xxl_path), _b(cfg.vae_path), _b(cfg.taesd_path), _b(cfg.backend),
            _b(cfg.params_backend), _b(cfg.max_vram), _b(cfg.split_mode), _b(cfg.model_args),
            cfg.n_threads, int(cfg.auto_fit), int(cfg.stream_layers), int(cfg.eager_load),
            int(cfg.enable_mmap), int(cfg.diffusion_flash_attn), int(cfg.diffusion_conv_direct),
            int(cfg.vae_conv_direct),
        )
    ...
    def generate(self, cfg: GenerationConfig) -> GeneratedImage:
        native = _GenerateConfig(
            ...
            cfg.width, cfg.height, cfg.steps, cfg.cfg_scale, cfg.distilled_guidance, cfg.seed,
            cfg.batch_count, cfg.clip_skip, int(cfg.vae_tiling), cfg.vae_tile_size_x,
            cfg.vae_tile_size_y, cfg.vae_target_overlap,
        )
        out = _Image()
        if not self._dll.phx_sd_generate(self._handle, ctypes.byref(native), ctypes.byref(out)):
            raise PhoenixSdError(self._error())
        try:
            pixels = ctypes.string_at(out.data, out.data_size)
            return GeneratedImage(int(out.width), int(out.height), int(out.channels), pixels)
        finally:
            self._dll.phx_sd_free_image(ctypes.byref(out))
