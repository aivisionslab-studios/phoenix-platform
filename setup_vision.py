...
class MtmdDriver:
    ...
    def _find_executable(self) -> str | None:
        repo_dir = self._project_root / "src" / "phoenix-llama-runtime"
        exe_names = (
        ...
        for name in exe_names:
            candidates = [
                self._project_root / "bin" / "phoenix-llama-runtime" / "windows-x64" / name,
    ...
    async def execute(self, plan: ExecutionPlan) -> ExecutionResult:
        ...
        if not exe_path:
            return ExecutionResult(
                ...
                errors=["llama-mtmd-cli nao encontrado. Compile o Phoenix Llama Runtime com Vulkan (GGML_VULKAN=ON)."],
            )
        ...
        cmd = [
            ...
            "-i", str(image_path),
            "-p", prompt,
            "-ngl", "0",  # CPU estrita - preserva a GPU para Phoenix Diffusion
            "--device", "none",
            "--no-op-offload",
            "-c", "4096",
        ]
