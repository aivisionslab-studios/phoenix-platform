...
_LABEL_PATTERNS = [
    ...
    (re.compile(r"(?i)\bmarca\b[^:\n]*[:\-]\s*([^\n|;]+)"), "brand", lambda x: FieldEvidence(value=x.strip(), status=ConfidenceStatus.PROBABLE, source="label", reason="Marca")),
    (re.compile(r"(?i)\btags\b[^:\n]*[:\-]\s*([^\n]+)"), "tags", lambda x: FieldEvidence(value=x.strip(), status=ConfidenceStatus.PROBABLE, source="label", reason="Tags")),
]

def extract_fields(block, doc_format, table_header=None):
...
def _extract_by_table_position(block, table_header):
    ...
    line = block.lines[0]
    if "\t" in line and line.count("\t") >= len(table_header)-1:
        cells = line.split("\t")
    elif ";" in line and line.count(";") >= len(table_header)-1:
        cells = line.split(";")
    else:
...
def _extract_by_label(block):
    ...
    for pattern, field_type, norm_fn in _LABEL_PATTERNS:
        ...
        if m:
            ...
            if ev:
                if field_type == "price":
                    if not any(k in fields for k in ["retail_price","wholesale_price","cost_price","sale_price","compare_at_price"]):
