...
class AssetManager:
    ...
    def _resolve_target_dir(self, catalog: dict, asset_name: str) -> Path:
        ...
        # Catálogos históricos usam ``downloads/images`` e
        # ``downloads/audio`` como se fossem diretórios de categoria. O
        # código anterior anexava o último componente ao diretório canônico,
        # criando ``Models/Image/images`` e ``Models/Audio/audio``. Os
        # scanners procuram no diretório canônico, então o download parecia
        # concluído mas desaparecia da detecção. Colapsamos somente aliases da
        # própria categoria; subpastas reais (encoders, vae, Kokoro) continuam.
        category_aliases = {
            "image": {"image", "images"},
            "audio": {"audio", "audios"},
            "voice": {"voice", "voices"},
        ...
        normalized_category = str(category or "Image").strip().lower()
        if subfolder.lower() in category_aliases.get(normalized_category, set()):
            subfolder = ""
        return (base / subfolder) if subfolder else base

    def describe_asset(self, asset_name: str, _seen: set[str] | None = None) -> dict:
        """Public read-only description of a catalog asset and its resolved destination.

        Keeps catalog/path resolution inside AssetManager so application services never
        need to reach into ``_load_catalog`` or ``_resolve_target_dir`` internals.
        This method never creates directories and never starts a download.
        """
        asset_name = (asset_name or "").strip()
        if not asset_name:
            return {"found": False, "error": "ID de asset vazio."}
        seen = set(_seen or ())
        if asset_name in seen:
            return {"found": False, "installed": False, "error": f"Ciclo de dependencias detectado em '{asset_name}'."}
        seen.add(asset_name)
        try:
            catalog = self._load_catalog(asset_name) or {}
        except Exception as exc:
            return {"found": False, "error": str(exc)}
        if not catalog:
            return {"found": False, "error": f"Asset '{asset_name}' não encontrado no catálogo."}
        if catalog.get("enabled") is False:
            return {
                "found": True,
                "enabled": False,
                "installed": False,
                "error": str(catalog.get("disabled_reason") or "Asset desativado pela política do produto."),
            }
        try:
            ...
            return {"found": True, "installed": False, "error": str(exc)}
        own_file_installed = bool(dest is not None and dest.is_file() and dest.stat().st_size > 0)
        dependency_states = []
        for dependency in catalog.get("dependencies", []):
            dependency_states.append(self.describe_asset(str(dependency), seen))
        missing_dependencies = [
            str(state.get("asset_id") or state.get("filename") or state.get("error") or "dependencia desconhecida")
            for state in dependency_states
            if not state.get("installed")
        ]
        installed = own_file_installed and not missing_dependencies
        return {
            "found": True,
            "asset_id": asset_name,
            "filename": filename,
            ...
            "installed": installed,
            "own_file_installed": own_file_installed,
            "dependencies": dependency_states,
            "missing_dependencies": missing_dependencies,
            "category": catalog.get("category"),
            "provider": catalog.get("provider", "http"),
        }
    ...
    def get_asset(self, asset_name: str) -> str:
        ...
        if catalog.get("enabled") is False:
            self.last_error = str(
                catalog.get("disabled_reason")
                or f"Asset '{asset_name}' desativado pela política do produto."
            )
            logger.warning("[AssetManager] %s", self.last_error)
            return None
