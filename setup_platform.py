...
PLATFORM_SOURCE = PROJECT_ROOT / "platform_source"
...
def _trusted_release_server() -> bool:
    """Confirma o dist registrado no build aceito.

    Arquivos extraídos de ZIP não preservam uma ordem de mtime confiável no
    Windows. Quando tamanho e SHA-256 batem com o manifesto, o dist é a fonte
    assinada/aceita e não deve ser recompilado só porque um .ts parece mais
    novo pelo relógio do arquivo.
    """
    manifest_path = PROJECT_ROOT / "release_build_manifest.json"
    if not (manifest_path.is_file() and SERVER_ENTRY.is_file()):
        return False
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        component = (manifest.get("components") or {}).get("aviary_server") or {}
        expected_size = int(component.get("size") or -1)
        expected_hash = str(component.get("sha256") or "").lower()
        if SERVER_ENTRY.stat().st_size != expected_size or len(expected_hash) != 64:
            return False
        actual_hash = hashlib.sha256(SERVER_ENTRY.read_bytes()).hexdigest()
...
def build_platform() -> tuple[bool, str]:
    ...
    install_cmd = ["npm", "ci"] if (PLATFORM_SOURCE / "package-lock.json").is_file() else ["npm", "install"]
    result = _run(install_cmd, cwd=PLATFORM_SOURCE)
    if result.returncode != 0:
        return False, f"'{' '.join(install_cmd)}' falhou: {result.stderr[-2000:]}"
