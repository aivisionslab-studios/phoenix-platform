...
function isUsable(p, diskModels = []) {
  const providerModels = p.type === "llama-server" ? [...new Set([...(p.models || []), ...diskModels])] : (p.models || []);
  return p.type === "gemini" || p.status === "connected" || (p.type === "llama-server" && providerModels.length > 0) || (p.status === "error" && providerModels.length > 0);
}

check("a condição isUsable usa inventário do llama mesmo offline",
  aviarySrc.includes("p.type === 'llama-server' && providerModels.length > 0"));
...
// Novo cenário real: runtime não compilou/porta 8081 offline, mas o disco
// contém GGUFs confirmados. Eles DEVEM continuar no seletor.
const llamaOfflineComDisco = { type: "llama-server", status: "disconnected", models: [] };
check("llama-server offline + GGUF confirmado no disco: continua VISÍVEL",
  isUsable(llamaOfflineComDisco, ["Qwen3-4B-Q4_K_M"]));
