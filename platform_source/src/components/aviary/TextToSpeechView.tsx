...
export const TextToSpeechView = () => {
  ...
  const handleGenerateAudiobook = async () => {
    ...
    // padrão do registry unificado de workflows. Se uma leitura falhar (rede
    ...
    const pollInterval = window.setInterval(async () => {
      try {
        const r = await fetch('/api/workflows/progress?include_idle=1&limit=50');
        if (r.ok) {
          const data = await r.json();
          const workflow = Array.isArray(data.workflows)
            ? data.workflows.find((item: any) => item?.kind === 'audiobook' && item?.active) ||
              data.workflows.find((item: any) => item?.kind === 'audiobook')
            : null;
          if (workflow?.detail) setDocProgress(workflow.detail);
        }
      } catch {
