...
// Mantemos os registros antigos no código-fonte para auditoria, mas modelos
// retirados da linha operacional não aparecem mais no produto ativo.
const ACTIVE_BENCHMARK_ITEMS = BENCHMARK_ITEMS.filter((item) => {
  const id = item.id.toLowerCase();
  return !id.includes('flux') && !id.includes('sd35');
});
...
export const BenchmarkSection = () => {
  ...
  const filteredItems = ACTIVE_BENCHMARK_ITEMS.filter((item) => {
    let matchesCat = true;
  ...
  const successCount = ACTIVE_BENCHMARK_ITEMS.filter(i => i.status === 'success').length;
  const failureCount = ACTIVE_BENCHMARK_ITEMS.filter(i => i.status !== 'success').length;
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-[#262d35] pb-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center text-xs">
          <div className="bg-[#0b0d10] p-2 rounded-lg border border-[#262d35]">
            ...
            <span className="font-bold text-white text-sm">{ACTIVE_BENCHMARK_ITEMS.length}</span>
          </div>
