...
export const HardwareDrawer: React.FC<HardwareDrawerProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchAhdeData = async () => {
      try {
        // PHX-PHASE6N: one aggregate read instead of three independent GETs
        // every 5s. The legacy endpoints remain available in the backend.
        const res = await fetch('/api/ahde/overview?limit=50', { cache: 'no-store' });
        const data = await res.json().catch(() => null);
        if (!res.ok || !data) {
          throw new Error((data && (data.detail || data.error)) || `Engine respondeu ${res.status}`);
        }
