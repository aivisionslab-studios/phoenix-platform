...
// POST /api/web/research em server.ts - a ponte que faltava entre o chat
...
async function main() {
  ...
  const fakeEngineOk = await startFakeEngine((req, res, body) => {
    if (req.url === "/api/web/research" && req.method === "POST") {
      lastReceivedBody = JSON.parse(body);
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end(JSON.stringify({ ok: true, query: lastReceivedBody.query, sources: [{ source_id: "S1", title: "Resultado real A", url: "https://example.com", page_text: "Conteúdo real A" }] }));
      return;
    }
  ...
  try {
    const r1 = await fetch(`http://localhost:${AVIARY_PORT}/api/web/research`, {
      method: "POST",
    ...
    check("corpo repassado de verdade (contém 'Resultado real A')", Array.isArray(data1.sources) && data1.sources[0]?.title === "Resultado real A");
    ...
    const r2 = await fetch(`http://localhost:${AVIARY_PORT}/api/web/research`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const r3 = await fetch(`http://localhost:${AVIARY_PORT}/api/web/research`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
