...
const finalExprMatch = appSrc.match(
  /const legacyImageGenerationIntent = (hasImageGenerationVerbNounInNonQuestionClause\(text\) \|\| looksLikeRawImagePrompt\(text\));[\s\S]*?const isImageGenerationIntent = arbiterClaimed[\s\S]*?\? arbiterDecision\?\.intent === 'image_generation'[\s\S]*?: legacyImageGenerationIntent;/
);
...
check(
  "NOVO: dispara no comando físico sem a palavra imagem",
  isImageGenerationIntent_NEW("criar uma phoenix saindo de vulcão em 8k") === true
);
check(
  "NOVO: dispara em cena natural sem resolução",
  isImageGenerationIntent_NEW("Crie um dragão voando sobre um castelo") === true
);
...
check(
  "NOVO: NÃO dispara em pergunta visual sem a palavra imagem",
  isImageGenerationIntent_NEW("Como criar uma phoenix saindo de um vulcão em 8k?") === false
);
check(
  "NOVO: NÃO dispara em criação de vídeo 8k",
  isImageGenerationIntent_NEW("Crie um vídeo de uma phoenix voando em 8k") === false
);
