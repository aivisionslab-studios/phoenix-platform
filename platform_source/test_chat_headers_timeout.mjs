...
function startFakeEngineServer(delayMs) {
  const server = http.createServer((req, res) => {
    let body = "";
    req.on("data", (c) => (body += c));
    req.on("end", async () => {
      if (req.url === "/api/engine/chat" && req.method === "POST") {
        const payload = JSON.parse(body || "{}");
        await sleep(delayMs);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({
          ok: true, deliver: true, text: "Resposta protegida depois do delay controlado.",
          reasoning: "", effectiveMode: "CPU", status: "DELIVERED_CPU",
          latencySeconds: delayMs / 1000, validationTrace: [],
          notice: { message: "Inferência validada via CPU." },
          usage: { prompt_tokens: 10, completion_tokens: 20 }, model: payload.model,
        }));
        return;
      }
      res.writeHead(200, { "Content-Type": "application/json" });
  ...
  return new Promise((resolve) => server.listen(FAKE_ENGINE_PORT, "127.0.0.1", () => resolve(server)));
}
...
async function startAviaryServer() {
  const child = spawn("node", [WRAPPER_PATH], {
    ...
    env: {
      ...process.env,
      NODE_ENV: "production",
      PORT: String(AVIARY_PORT),
      PHOENIX_ENGINE_URL: `http://127.0.0.1:${FAKE_ENGINE_PORT}`,
    },
...
async function attemptChatWithSlowBackend(delayMs) {
  ...
  const phoenixEngine = await startFakeEngineServer(delayMs);
...
async function main() {
  ...
  try {
    ...
    console.log("\n-- Confirmando o gateway protegido da R6.2 --");
    const source = fs.readFileSync(SERVER_TS_PATH, "utf-8");
    check("llama-server local passa obrigatoriamente por /api/engine/chat", source.includes('/api/engine/chat'));
    check("a decisão do Forge chega ao Aviary no envelope phoenixGuard", source.includes('phoenixGuard'));
  } finally {
